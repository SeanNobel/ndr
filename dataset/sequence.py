...
class TypedAccuracyCounter:
    ...
    def get(self) -> Dict[str, Any]:
        return {
            f"accuracy/{self.type_names[t]}": data.get() for t, data in self.counters.items()
        }
...
class SampleTracker:
    ...
    def get(self, limit: Optional[int] = None) -> framework.visualize.plot.TextTable:
        return framework.visualize.plot.TextTable(
            [n for n, e in zip(self.header, self.enabled) if e], self.pick_seqeunces(limit)
        )
...
class SampleTrackerTestState(SequenceTestState):
    def __init__(
        ...
        self.bad_sequences = SampleTracker(
            ["Input", "Reference", "Type", "Output"] + self.extra_ok_columns()
        )
    ...
    def get_gt_sample(
        self, data: Dict[str, torch.Tensor], i: int, *args, **kwargs
    ) -> List[str]:
    ...
    def get_failed_sample(
        self, net_out: Any, data: Dict[str, torch.Tensor], i: int, *args, **kwargs
    ) -> List[str]:
    ...
    def track_samples(
        self, net_out: Any, ok_mask: torch.Tensor, data: Dict[str, Any], *args, **kwargs
    ):
        if self.max_bad_samples > 0:
            for i in torch.nonzero(~ok_mask).squeeze(-1):
                self.bad_sequences.add(
                    *self.get_failed_sample(net_out, data, i, *args, **kwargs)
                )
...
class TextClassifierTestState(SampleTrackerTestState):
    def __init__(
        self,
        input_to_text: Callable[[torch.Tensor], str],
        output_to_text: Callable[[torch.Tensor], str],
        batch_dim: int = 1,
        max_bad_samples: int = 100,
        type_names: Optional[List[str]] = None,
        max_good_samples: int = 0,
...
class TextSequenceTestState(SampleTrackerTestState):
    ...
    def net_out_to_text(
        self, net_out: Optional[Tuple[torch.Tensor, Optional[torch.Tensor]]], _, i: int
    ) -> str:
    ...
    def get_failed_sample(
        ...
        data: Dict[str, torch.Tensor],
        i: int,
        prefix_ok: torch.Tensor,
        oracle_ok: torch.Tensor,
    ) -> List[str]:
    ...
    def step(
        ...
        if not self.is_index_tensor(scores):
            ...
            oracle_ok = self.compare_direct(
                (out_noeos, data["out_len"].clamp_(max=out_noeos.shape[1 - self.batch_dim])),
                data["out"],
                data["out_len"],
            )
    ...
    def plot(self) -> Dict[str, Any]:
        ...
        if self.losses:
            res["loss_histogram"] = framework.visualize.plot.Histogram(
                torch.cat(self.losses, 0)
            )
