...
class CompositionalTableLookup:
    ...
    class CLTData:
        ...
        def construct(
            ...
            indices: Dict[str, List[int]],
            in_vocabulary: Union[
                framework.data_structures.CharVocabulary,
                framework.data_structures.WordVocabulary,
            ],
            out_vocabulary: Union[
                framework.data_structures.CharVocabulary,
                framework.data_structures.WordVocabulary,
            ],
        ):
            ...
            self.word_based = isinstance(
                in_vocabulary, framework.data_structures.WordVocabulary
            )
    ...
    def format_cmd_str(self, command, arg):
        ...
        if self.reversed:
            return spacer.join(
                [self.bit_format(arg)] + [self.table_names[i] for i in reversed(command)]
            )
        else:
    ...
    def sample_data(
        self, n_gen: int, tables: np.ndarray, sampler_fn: Callable[[], Tuple[int, List[int]]]
    ...
    def generate_data(self) -> CLTData:
        ...
        n_possible_compositions = int(
            len(tables_all_compositions) ** 2
        ...
        for d in range(self.max_depth + 1):
            ...
            while len(test_compositions) < n_before + n_reserved:
                test_compositions.add(
                    tuple(s.randint(len(tables_all_compositions), size=[d]).tolist())
                )
        ...
        n_iid_train = (
            n_iid_train if self.max_n_samples is None else min(self.max_n_samples, n_iid_train)
        )
        ...
        def sample_iid():
            while True:
                ...
                comp = tuple(
                    s.randint(len(tables_all_compositions), size=[self.max_depth])[
                        : s.randint(2, self.max_depth + 1)
                    ].tolist()
                )
        ...
        i_iid, o_iid, depths = self.sample_data(
            n_iid_train + 2 * n_test, tables_all_compositions, sample_iid
        )
        offset = len(inputs)
        inputs += i_iid
        outputs += o_iid
        ...
        def sample_new_primitive_1():
            ...
            command = s.randint(len(tables) - 1, size=[self.max_depth])[
                : s.randint(2, self.max_depth + 1)
            ].tolist()
        ...
        offset = len(inputs)
        indices["train_new_primitive"] += [
            offset + i for i in range(len(i_new)) if i not in t_indices
        ]
        ...
        def sample_deeper_valid():
            return (
                s.randint(2**self.n_bits),
                s.randint(
                    len(tables_all_compositions),
                    size=[self.max_depth + self.n_more_depth_valid],
                )[
                    : s.randint(
                        self.max_depth + 1, self.max_depth + self.n_more_depth_valid + 1
                    )
                ].tolist(),
            )

        i_deeper, o_deeper, _ = self.sample_data(
            1000, tables_all_compositions, sample_deeper_valid
        )
        ...
        def sample_deeper():
            return (
                s.randint(2**self.n_bits),
                s.randint(
                    len(tables_all_compositions), size=[self.max_depth + self.n_more_depth]
                )[
                    : s.randint(
                        self.max_depth + 1 + self.n_more_depth_valid,
                        self.max_depth + self.n_more_depth + 1,
                    )
                ].tolist(),
            )
        ...
        if self.atomic_input:
            ...
            in_vocab = framework.data_structures.CharVocabulary(
                set(self.table_names) | {"0", "1"}
            )
    ...
    def __init__(
        ...
        reversed: bool = False,
        cache: str = "./cache",
        max_n_sampes: Optional[int] = None,
        atomic_input: bool = True,
        copy_input: bool = False,
        detailed_output: bool = True,
        n_more_depth_valid: int = 1,
    ) -> None:
        self.n_bits = n_bits
        ...
        self.table_names = (
            list(string.ascii_lowercase[:n_tables])
            if len(string.ascii_lowercase) >= n_tables
            else [f"c{n+1}" for n in range(n_tables)]
        )

        assert n_more_depth > n_more_depth_valid

        self.id = (
            f"{max_depth}_{n_tables}_{n_bits}_{n_more_depth}_{int(reversed)}_{n_more_depth_valid}_"
        ...
        if self.id not in self.cache:
            ...
            with framework.utils.LockFile(
                os.path.join(cache, self.__class__.__name__, "lock")
            ):
    ...
    def start_test(self) -> TextSequenceTestState:
        if self.atomic_input:
            ...
            return TextSequenceTestState(
                self.in_vocabulary, self.out_vocabulary, type_names=self.types
            )
