...
class MultiHeadAttentionBase(LayerWithVisualization):
    ...
    def apply_logit_masks(
        ...
        if mask.src_length_mask is not None:
            ...
            logits = logits.masked_fill(
                mask.src_length_mask.view([b] + [1] * pad_dims + [i]), val
            )
    ...
    def _attention_read(
        self, mask: Optional[AttentionMask], scores: torch.Tensor, v: torch.Tensor
    ...
    def transform_data(
        self, input: torch.Tensor, proj: Callable[[torch.Tensor], torch.Tensor], n_projs: int
    ) -> List[torch.Tensor]:
        ...
        transformed = (
            proj(input)
            .view(n_batch, n_steps, self.n_heads, n_projs, -1)
            .permute(0, 2, 1, 3, 4)
            .contiguous()
            .view(n_batch * self.n_heads, n_steps, n_projs, -1)
        )
    ...
    def plot(self, options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if (
            options.get("mha.plot_head_details")
        ...
        r["attention_max"] = framework.visualize.plot.AnimatedHeatmap(
            torch.stack(
                [layer.max(0)[0] for _, layer in enumerate(self.attention_to_visualize)], 0
            ),
            ylabel="dest",
            xlabel="src",
            textval=False,
...
class AttentionMergeMixin:
    def __init__(self, out_size: Optional[int]) -> None:
        self.multi_head_merge = torch.nn.Linear(
            self.n_heads * self.projection_size, out_size or self.state_size, bias=False
        )

    def merged_attention(
        self, n_batch: int, n_out_steps: int, *args, need_weights: bool = False, **kwargs
...
class AbsPosAttentionBase(MultiHeadAttentionBase):
    def get_attention_scores(
        self, mask: Optional[torch.Tensor], q: torch.Tensor, k: torch.Tensor
    ) -> torch.Tensor:
        logits = torch.bmm(q, k.transpose(1, 2))
        return self._masked_softmax(logits * self.scale, mask)

    def _attention(
        self, mask: Optional[torch.Tensor], q: torch.Tensor, k: torch.Tensor, v: torch.Tensor
    ) -> torch.Tensor:
...
class MultiHeadAttention(AttentionMergeMixin, AbsPosAttentionBase):
    def __init__(
        ...
        self.data_to_q = torch.nn.Linear(
            input_size or state_size, n_heads * self.projection_size, bias=False
        )
