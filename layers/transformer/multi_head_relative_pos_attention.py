...
def shift(posmat: torch.Tensor) -> torch.Tensor:
    ...
    p = p.narrow(-1, posmat.shape[-1] // 2, posmat.shape[-1] * posmat.shape[-2]).view_as(
        posmat
    )
...
class RelativeAttentionBase(MultiHeadAttentionBase):
    ...
    def get_attention_scores(
        ...
        k_pos: torch.Tensor,
        pos_offset: int,
        ar_gate: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        ...
        pos = torch.matmul(
            q_pos.view(n_batch, self.n_heads, n_out_steps, -1),
            self.dropout(k_pos).transpose(-1, -2),
        )  # [n_batch, n_head, n_out, n_in * 2 - 1]
        fpos = shift(pos).flatten(0, 1)
        if ar_gate is not None:
            fpos = fpos * ar_gate + pos.flatten(0, 1)[..., fpos.shape[-1] - 1 :] * (
                1 - ar_gate
            )

        # return self._masked_softmax((fpos) * self.scale, mask)
        if self.visualization_enabled:
            self.vis_pos_vs_content.append(
                (
                    content.view(n_batch, self.n_heads, *content.shape[1:])[0] * self.scale,
    ...
    def _attention(
        ...
        scores = self.get_attention_scores(
            mask, q_content, k_content, q_pos, k_pos, pos_offset, ar_gate
        )
    ...
    def plot(self, options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if options.get("mha.plot_head_details") and self.vis_pos_vs_content:
            for head in range(self.vis_pos_vs_content[0][0].shape[0]):
                cont = torch.stack(
                    [layer[0][head] for _, layer in enumerate(self.vis_pos_vs_content)], 0
                )
                pos = torch.stack(
                    [layer[1][head] for _, layer in enumerate(self.vis_pos_vs_content)], 0
                )
                i = torch.stack(
                    [layer[head] for _, layer in enumerate(self.attention_to_visualize)], 0
                )
                ...
                r[f"content_vs_pos_{head}"] = framework.visualize.plot.AnimatedHeatmap(
                    color,
                    ylabel="dest",
                    xlabel="src",
                    textval=False,
                    ...
                    cmap="brg",
                    colorbar=True,
                    colorbar_ticks=[0, 0.99],
                    colorbar_labels=["pos", "con"],
...
class FixedRelativeMultiheadAttentionBase(RelativeAttentionBase):
    ...
    def _create_buffer(self, max_len: int):
        return framework.layers.sinusoidal_pos_embedding(
            self.state_size, 2 * max_len - 1, -max_len + 1, device=self.pos_to_pq.weight.device
        )
...
class FixedRelativeMultiheadAttention(
    AttentionMergeMixin, FixedRelativeMultiheadAttentionBase
):
    def __init__(
        self,
        state_size: int,
        ...
        super(AttentionMergeMixin, self).__init__(
            state_size, n_heads, dropout, input_size, projection_size=projection_size
        )
        ...
        self.data_to_q = torch.nn.Linear(
            self.input_size, n_heads * self.projection_size, bias=False
        )
        self.data_to_absgate = (
            torch.nn.Linear(self.input_size, n_heads) if absolute_gate else None
        )
    ...
    def add_head_specific_bias(
        ...
        return (
            (
                data.view(-1, bias.shape[0], *data.shape[1:]) + bias.unsqueeze(1).type_as(data)
            ).view_as(data)
            if bias is not None
            else data
        )
    ...
    def forward(
        ...
        k_pos = (
            self.get_pos(in_len, pos_offset)
        ...
        absgate = (
            torch.sigmoid(self.transform_data(curr_state, self.data_to_absgate, 1)[0])
            if self.data_to_absgate is not None
            else None
        )
