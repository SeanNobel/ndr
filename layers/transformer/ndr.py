...
from ndr.layers.transformer.multi_head_relative_pos_attention import (
    FixedRelativeMultiheadAttentionBase,
)
...
class UniversalTransformerRandomLayerEncoder(torch.nn.Module):
    def __init__(
        self, layer, n_layers: int, n_extra: int, n_test: Optional[int] = None, *args, **kwargs
    ):
        super().__init__()
    ...
    def forward(self, data: torch.Tensor, *args, **kwargs):
        self.set_n_layers(
            np.random.randint(self.n_layers, self.n_extra + self.n_layers + 1)
            if self.training
            else (self.n_test or self.n_layers)
        )
...
def UniversalTransformerRandomLayerEncoderWithLayer(layer):
    return lambda *args, **kwargs: UniversalTransformerRandomLayerEncoder(
        layer, *args, **kwargs
    )
...
class NDRResidual(RegularizedLayer, LayerWithVisualization):
    def __init__(
        ...
        **kwargs
        ...
        self.att = FixedRelativeMultiheadAttention(
            d_model, nhead, dropout=attention_dropout, absolute_gate=abs_gate
        )
    ...
    def forward(self, src: torch.Tensor, mask: Optional[AttentionMask] = None) -> torch.Tensor:
        ...
        if self.training and self.p_gate_drop > 0:
            bgate = bgate.masked_fill(
                torch.rand(*bgate.shape[:-1], 1, device=bgate.device, dtype=bgate.dtype)
                < self.p_gate_drop,
                0,
            )
    ...
    def reset_parameters(self):
        torch.nn.init.xavier_uniform_(
            self.p1.weight, gain=torch.nn.init.calculate_gain("relu")
        )
        # torch.nn.init.xavier_uniform_(self.p2.weight)

        torch.nn.init.xavier_uniform_(
            self.g1.weight, gain=torch.nn.init.calculate_gain("relu")
        )
        torch.nn.init.xavier_uniform_(
            self.g2.weight, gain=torch.nn.init.calculate_gain("sigmoid")
        )
