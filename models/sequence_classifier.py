...
class SequenceClassifier(torch.nn.Module):
    def __init__(
        ...
        self.encoder = encoder(
            in_vocabulary_size + int(n_thinking_steps > 0),
            hidden_size,
            n_layers,
            embedding_size or hidden_size,
            dropout,
            lstm,
        )
    ...
    def __call__(self, input: torch.Tensor, in_lengths: torch.Tensor) -> torch.Tensor:
        # Input: [T, N]
        input, in_lengths = add_thinkig_steps(
            input, in_lengths, self.n_thinking_steps, self.think_token
        )
