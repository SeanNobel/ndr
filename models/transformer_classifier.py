...
from ndr.layers import TiedEmbedding
from ndr.layers.transformer import TransformerEncoderWithLayer, AttentionMask, Transformer
from typing import Callable, Optional
import math
from .encoder_decoder import add_eos
from ndr.layers.transformer.multi_head_attention import MultiHeadAttention
...
class TransformerClassifierModel(torch.nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.pos = pos_embeddig or framework.layers.PositionalEncoding(
            state_size,
            max_len=max_len,
            batch_first=True,
    ...
    def construct(self, transformer, **kwargs):
        ...
        if self.out_mode == "tied":
            ...
            self.output_map = MultiHeadAttention(
                self.state_size, 1, out_size=self.n_out_tokens
            )
            self.out_query = torch.nn.Parameter(
                torch.randn([1, self.state_size]) / math.sqrt(self.state_size)
            )
    ...
    def get_result(self, res: torch.Tensor, src_len: torch.Tensor) -> torch.Tensor:
        if self.result_column == "first":
            ...
            return res.gather(
                1, src_len.view([src_len.shape[0], 1, 1]).expand(-1, -1, res.shape[-1]) - 1
            ).squeeze(1)
        else:
            assert False
    ...
    def forward(self, src: torch.Tensor, src_len: torch.Tensor) -> torch.Tensor:
        ...
        if self.out_mode == "attention":
            return self.output_map(
                self.out_query.expand(src.shape[0], -1, -1),
                res,
                mask=AttentionMask(in_len_mask, None),
            ).squeeze(1)
        else:
